...
func Test_buildTableFrames_withProvidedLogData(t *testing.T) {
	...
	frames, err := buildTableFrames(
		...
		buildExactMessageIndex(queryModel, logResult, createLogTopicMappings(functions)),
	)
	if err != nil {
...
func Test_fetchLog_usesDefaultLargePageLimit(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if got := r.URL.Query().Get("include_total"); got != "false" {
			t.Fatalf("expected include_total=false, got %q", got)
		}
		if got := r.URL.Query().Get("from"); got != "1772027261" {
			t.Fatalf("expected from=1772027261, got %q", got)
		}
		if got := r.URL.Query().Get("to"); got != "1772027271" {
			t.Fatalf("expected to=1772027271, got %q", got)
		}
	...
	_, err := fetchLog(t.Context(), instance, request, []string{"obj/generated/topic"})
	if err != nil {
		t.Fatalf("fetchLog returned error: %v", err)
	}
